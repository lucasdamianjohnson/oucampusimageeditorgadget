...
##
    ...
    max-width:none;
        ...
        <body class="body-pad">
          ...
          <form method="post">
            ...
            <div class="form-group"><button id = "recommended-dims" class="btn">Use Recommended Aspect</button><button class="btn margin-left" id = "whole-image">Select Whole Image</button></div>
  ...
  .preview img{
    max-width:none;
        ...
        <body  class="body-pad">
          ...
          ?>
          <form method="post">
            <div class="form-group margin-top"><input value = "Previous" type = "submit" name = "previous" class="btn" /><input value = "Exit" class="btn margin-left" type = "submit" name = "exit"/><input class="btn margin-left btn-primary" value = "Confirm" type = "submit" name = "confirm"/></div>
            <input type = "hidden" name = "stage" value = "<?php echo $_SESSION["current_stage"] ?>">
          </form>
        </body>
      </html>
  ...
  function optimizeImages($images){
	  ...
	  if(file_get_contents('http://www.mtu.edu/mtu_resources/php/cms/getHttps/status.php?url=http://kraken.io') != 301) {
        ...
        if (curl_multi_select($mh) != -1) {
            do {
              ...
              if($ch && curl_getinfo($ch, CURLINFO_HTTP_CODE) == 423){
                ...
                echo "Warning: The file ".basename($url)." could not be uploaded, because the image is currently checked in.<br>";
              }
